...
export class AdminService {
  ...
  public isAuthenticated(allowRoles: string[]):boolean{         // si existe un token de inicio de sesión, permitira acceder a inicio
    
    const token = localStorage.getItem('token');

    if(!token){           // si no existe un token , retorna false
      return false;
    }
